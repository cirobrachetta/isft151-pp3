// Controller: orquesta reglas y responde DTO
...
const UserController = {
    register(req, res) {
        try {
            ...
            res.status(201).json({ message: 'ok', user: user.toDTO() });
        } catch (e) { 
  ...
  update(req, res) {
    try {
      ...
      const hash = req.body.password ? bcrypt.hashSync(req.body.password, 10) : current.passwordHash;
  ...
  list(_req, res) {
    try {
      res.json(UserDAO.selectUsers().map(u => u.toDTO()));
    } catch (e) { 
        res.status(400).json({ error: e.message }); 
    }
  },
